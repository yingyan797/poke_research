...
class MoveQuery(PokemonAttributeQuery):
    def __init__(self, keys=[]):
        ...
        print(pb.move("tackle").__dict__.keys())
        # print(pb.move("tackle").contest_effect)
    def find_pokemon(self):
        return {key: [value.name for value in pb.move(key).learned_by_pokemon] for key in self.keys}
    
    def accuracy(self):
        return {key: pb.move(key).accuracy for key in self.keys}
    def contest_effect(self):
        return {key: pb.move(key).contest_effect.name for key in self.keys}
    def super_contest_effect(self):
        return {key: pb.move(key).super_contest_effect.name for key in self.keys}
    def contest_type(self):
        return {key: pb.move(key).contest_type for key in self.keys}
    def damage_class(self):
        return {key: pb.move(key).damage_class for key in self.keys}
    
    def effect_chance(self):
        return {key: pb.move(key).effect_chance for key in self.keys}
    def effect_changes(self):
        return {key: pb.move(key).effect_changes for key in self.keys}
    def effect_entries(self):
        return {key: [entry.__dict__ for entry in pb.move(key).effect_entries] for key in self.keys}
    def flavor_text_entries(self):
        return {key: [entry.__dict__ for entry in pb.move(key).flavor_text_entries] for key in self.keys}
    def contest_combos(self):
        return {key: {ck: {u: [uv.name for uv in uvs] if uvs else [] for u, uvs in cv.__dict__.items()} for ck, cv in pb.move(key).contest_combos.__dict__.items()} for key in self.keys}
    def machines(self):
        return {key: pb.move(key).machines for key in self.keys}
    def power(self):
        return {key: pb.move(key).power for key in self.keys}
    def pp(self):
        return {key: pb.move(key).pp for key in self.keys}
    def priority(self):
        return {key: pb.move(key).priority for key in self.keys}
    def stat_changes(self):
        return {key: [change.__dict__ for change in pb.move(key).stat_changes] for key in self.keys}
    def target(self):
        return {key: pb.move(key).target.name for key in self.keys}
    def move_type(self):
        return {key: pb.move(key).type.name for key in self.keys}

if __name__ == "__main__":
    print(json.loads("[\"ffd\"]"))
